...
q10_32_19_2 <- 
  ...
  q10_32_19_2 <- na.omit(surveys_combined_demos_limited) %>% dplyr::group_by(survey_label,`10_32_19_2`) %>% 
  dplyr::summarize(count=dplyr::n()) %>% mutate(perc=count/sum(count)) %>%
  ggplot(mapping = aes(x =survey_label,y = perc, fill = `10_32_19_2`)) +
...
#              position = position_jitter(0.21)) +
  ...
  theme(plot.title = element_text(face="bold",size=24),
        ...
        axis.text.y = element_text(color="black",size=20, angle=0),
        axis.title.y = element_text(color="black",size=20, angle=90))
ggexport(plotlist = list(q10_32_19_2), filename = "plots/ q10_32_19_2.png", width = 1200, height = 800)
